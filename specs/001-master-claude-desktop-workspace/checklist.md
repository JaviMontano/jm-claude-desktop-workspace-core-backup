# Checklist

- [x] Separate source staging from new JM repos
- [x] Create private GitHub repos for core and workspace
- [x] Extract runtime assets into the core
- [x] Preserve MetodologIA as an optional pack
- [x] Model desktop MCP parity from the real config
- [x] Model Codex Google and GitHub connectors
- [x] Add scripts and repository tests
- [x] Bootstrap and verify the local workspace instance
- [x] Export Antigravity view
- [x] Push both repos
